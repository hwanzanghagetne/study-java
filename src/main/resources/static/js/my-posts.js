...
async function loadPosts(page) {
  const response = await fetch(`/api/members/me/posts?page=${page}&size=10&sort=createdAt,desc`);
  ...
  if (pageData.content.length === 0) {
    postList.innerHTML = "<li>아직 작성한 게시글이 없습니다.</li>";
  }
  ...
  pageData.content.forEach((post) => {
    ...
    li.innerHTML = `
      ...
      <span class="post-meta">조회 ${post.viewCount} · ${createdAt}</span>
    `;
